...
{
    $isBackoffice = (($_GET['back'] ?? '') === 'backoffice');
    $backUrl  = $isBackoffice ? '../../View/BackOffice/admin-feedback.php' : 'events.php';
    $backLabel = $isBackoffice ? 'Back to Backoffice' : 'Back to Events';
    ...
    echo '<a class="btn" href="' . htmlspecialchars($backUrl) . '">' . htmlspecialchars($backLabel) . '</a></div></body></html>';
}
...
<body>
  <div class="card">
    ...
    $userStmt = $db->prepare("SELECT userId, fullName, email FROM Users WHERE fullName = :name AND email = :email LIMIT 1");
