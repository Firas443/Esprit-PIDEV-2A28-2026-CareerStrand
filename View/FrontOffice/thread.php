...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $defaultUserId !== null) {
    ...
    if ($action === 'analyze_submission' || $action === 'create_submission') {
        ...
        if ($targetChallengeId > 0 && !array_filter($submissionErrors)) {
            if ($member === null) {
                $status = 'not_member';
            } elseif ($engagementController->getSubmissionByMemberAndChallenge((int) $member['groupMemberId'], $targetChallengeId) !== null) {
        ...
        if ($status === 'created' || $status === 'not_member' || $status === 'already_submitted') {
...
$submissionFeedback = match ($submissionNotice) {
    ...
    'already_submitted' => 'You already submitted work for this challenge. Each member can submit only once.',
...
<body>
    <div class="page-shell">
        <?php
            $activePage = 'skillhub';
            $brandSubtitle = 'thread workspace';
                        ...
                        <section class="thread-view" id="overviewView">
                            ...
                            <div class="content-grid">
                                ...
                                <article class="panel submit-panel">
                                    ...
                                    <?php } elseif ($latestSubmission !== null) { ?>
                                        <div class="submission-actions">
                                            <p>You already submitted this challenge. Each member can submit only once.</p>
                                            <?php if (!empty($latestSubmission['projectLink'])) { ?>
                                                <a class="primary-btn full" href="<?= h($latestSubmission['projectLink']); ?>" target="_blank" rel="noopener noreferrer">Open your submission</a>
                                            <?php } ?>
                                        </div>
                                    <?php } else { ?>
                                        <div class="submission-actions">
