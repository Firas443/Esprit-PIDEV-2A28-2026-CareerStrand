...
$pendingSubmissionCounts = $engagementController->getPendingSubmissionCountsByChallenge();
...
$workItems = array_map(static function (array $workItem) use ($hubMap, $userMap, $threadCountsByChallenge, $pendingSubmissionCounts): array {
    $hub = $hubMap[(int) $workItem['groupId']] ?? ['name' => 'Skill Hub'];
    $manager = $userMap[(int) $workItem['managerId']] ?? ['fullName' => 'Unknown', 'role' => 'user'];
    $challengeId = (int) $workItem['challengeId'];
    $workItem['hubName'] = $hub['name'] ?? 'Skill Hub';
    $workItem['managerName'] = $manager['fullName'] ?? 'Unknown';
    $workItem['threadCount'] = $threadCountsByChallenge[$challengeId] ?? 0;
    $workItem['pendingSubmissionCount'] = $pendingSubmissionCounts[$challengeId] ?? 0;
    return $workItem;
}, $workRows);
                            ...
                            <?php } else { ?>
                                <?php foreach ($filteredWorkItems as $item) { ?>
                                    <tr>
                                        ...
                                        <td><span class="status-chip <?= (int) $item['pendingSubmissionCount'] > 0 ? 'status-review' : 'status-published'; ?>"><?= h((string) $item['pendingSubmissionCount']); ?></span></td>
                                        <td>
