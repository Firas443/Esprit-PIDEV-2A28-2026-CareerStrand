...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && ($challengeId <= 0 || $challenge === null)) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $postedSubmissionId > 0) {
    foreach ($submissions as $submission) {
        if ((int) $submission['submissionId'] !== $postedSubmissionId) {
            continue;
        }
        ...
        if (isPendingSubmission($submission)) {
            ...
            $editingSubmissionId = $postedSubmissionId;
            $reviewSubmissionId = 0;
        }
        break;
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'generate_ai_feedback') {
    ...
    if ($challengeId > 0) {
        ...
        $submissions = $engagementController->getSubmissionsByChallenge($challengeId);
    }
...
<body>
    <div class="admin-shell">
        <?php
        $adminSidebarCardTitle = 'Review workflow';
        $adminSidebarCardText = 'Focus on one submission at a time: inspect the work, score it once, then move it into the reviewed archive.';
        ?>
                    ...
                    <article class="panel review-detail-panel">
                        ...
                        <?php } else { ?>
                            <div class="review-detail-grid">
                                ...
                                <form
                                    method="POST"
                                    action="admin-skillhub-reviews.php?challengeId=<?= (int) $challengeId; ?><?= $isEditingReviewed ? '&editSubmission=' : '&reviewSubmission='; ?><?= (int) $activeSubmission['submissionId']; ?>"
                                    novalidate
                                    class="submission-evaluation-form review-score-panel"
                                >
