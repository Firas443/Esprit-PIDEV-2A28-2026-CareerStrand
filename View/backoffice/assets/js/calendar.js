const COURSES = [
  {id:1, title:'Portfolio Foundations',       track:'Identity'},
  {id:2, title:'Intro to Digital Workflows',  track:'Learning'},
  {id:3, title:'Career Readiness Sprint',     track:'Opportunity'},
  {id:4, title:'UI/UX Fundamentals',          track:'Design'},
  {id:5, title:'Storytelling for Careers',    track:'Communication'},
  {id:6, title:'Data Literacy Bootcamp',      track:'Technical'},
  {id:7, title:'Remote Work Essentials',      track:'Learning'},
  {id:8, title:'HTML & CSS Foundations',      track:'Technical'},
];
...
let state   = JSON.parse(sessionStorage.getItem('cs_cal') || '{}');
let now     = new Date();
let curYear = now.getFullYear();
let curMon  = now.getMonth();
let selDay  = null; // { year, month, day }
...
function renderCalendar() {
  ...
  for (let i = 0; i < totalCells; i++) {
    ...
    let pillsHtml = evs.slice(0,2).map(e => {
      const tc = TRACK_COLORS[e.track] || TRACK_COLORS.Identity;
...
function renderDayDetail() {
  ...
  el.innerHTML = evs.map(ev => {
    const tc   = TRACK_COLORS[ev.track] || TRACK_COLORS.Identity;
    ...
    return `
      <div class="detail-ev" style="border-left-color:${tc.border}">
        ...
        <div class="detail-ev-track">${ev.track} · ${days} jours au total</div>
...
function renderAllCourses() {
  ...
  el.innerHTML = COURSES.map(c => {
    const s  = state[c.id] || {};
    const tc = TRACK_COLORS[c.track] || TRACK_COLORS.Identity;
    const dateInfo = s.scheduled
      ? `<div class="course-item-dates">📅 ${fmtDate(s.start)}<br>→ ${fmtDate(s.end)}</div>`
      : `<div class="not-scheduled">Non planifié — <a href="admin-courses.php" style="color:var(--blue-2)">Planifier</a></div>`;
    return `
...
function renderLegend() {
  ...
  el.innerHTML = tracks.map(t => {
    const tc = TRACK_COLORS[t] || TRACK_COLORS.Identity;
