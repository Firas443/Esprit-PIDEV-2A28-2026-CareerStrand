...
.toast {
  ...
  z-index: 99999; /* ← higher than modal's 9999 */
  opacity: 0;
  transition: opacity .22s ease, transform .22s ease;
  white-space: nowrap;
}
    .toast.show { opacity: 1; transform: translateX(-50%) translateY(0); } //error css
...
const API_BASE = '/Careerstrand/controller/controller-opportunities/';
let allOppos        = [];
let filterStatus    = '';
...
async function loadOpportunities() {
  ...
  try {
    const res = await fetch(`${API_BASE}get_opportunities.php?source=back&${params}`);
...
// RENDER (contient edit et delete du tableau)
...
async function saveOpportunity() {
  ...
  const body = {
    ...
    title:         document.getElementById('fTitle').value.trim(), //trim remove espace inutile
