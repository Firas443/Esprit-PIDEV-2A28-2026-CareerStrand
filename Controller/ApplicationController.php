...
class ApplicationController {
    ...
    private function createApplication(): void {
        ...
        $db = $this->model->getDb();
        $this->model->ensureAutoIncrementId();
        ...
        $stmt = $db->prepare("INSERT INTO Application (userId, opportunityId, motivation, portfolio, compatibilityScore, status, appliedAt)
                               VALUES (:userId, :opportunityId, :motivation, :portfolio, :score, 'pending', NOW())");
        $stmt->execute([
    ...
    private function normalizeCreateBody(): void {
        $aliases = ['opportunity_id', 'opportunityID', 'id'];
        foreach ($aliases as $alias) {
            if (empty($this->body['opportunityId']) && !empty($this->body[$alias])) {
                $this->body['opportunityId'] = $this->body[$alias];
                break;
            }
        }

        if (isset($this->body['opportunityId'])) {
            $this->body['opportunityId'] = (int)$this->body['opportunityId'];
        }
        if (isset($this->body['userId'])) {
            $this->body['userId'] = (int)$this->body['userId'];
        }
    }
    ...
    private function summarizeApplication(): void {
        ...
        $apiKey = trim((string) (getenv('HF_TOKEN') ?: (getenv('HUGGINGFACE_API_KEY') ?: config::getHuggingFaceApiKey())));
        if ($apiKey === '') {
            ob_end_clean();
            echo json_encode(['success' => true, 'data' => $this->buildLocalFitSummary([
        ...
        $input = $this->buildApplicationAiInput($app);
        try {
            $summary = $this->callHuggingFaceSummary($apiKey, $input);
        } catch (Throwable $e) {
            $summary = $this->buildLocalFitSummary($input);
            $summary['summary'] .= ' The external AI request failed, so this local review was generated instead. Error: ' . $e->getMessage();
        }
        ob_end_clean();
        echo json_encode(['success' => true, 'data' => $summary]);
    }

    private function buildApplicationAiInput(array $app): array {
        $portfolioText = '';
    ...
    private function buildLocalFitSummary(array $input): array {
        $motivation = trim((string)($input['motivation'] ?? ''));
        $description = trim((string)($input['opportunityDescription'] ?? ''));
        $portfolioUrl = trim((string)($input['portfolioUrl'] ?? ''));
        $score = is_numeric($input['compatibilityScore'] ?? null) ? (int)$input['compatibilityScore'] : 0;

        $summaryParts = [];
        $summaryParts[] = $motivation !== ''
            ? 'The applicant submitted motivation for this opportunity.'
            : 'The applicant did not provide a clear motivation statement.';
        $summaryParts[] = $description !== ''
            ? 'The opportunity description is available for manual review.'
            : 'The opportunity description is missing or very limited.';

        return $this->buildFitSummary([
            'compatibilityScore' => $score,
            'motivation' => $motivation,
            'portfolioUrl' => $portfolioUrl,
            'portfolioText' => '',
            'portfolioFetchNote' => $portfolioUrl !== ''
                ? 'AI API key is not configured, so portfolio content was not fetched for summarization.'
                : 'No portfolio URL was provided.',
        ], implode(' ', $summaryParts) . ' Configure HF_TOKEN or HUGGINGFACE_API_KEY for a generated AI summary.');
    }
    ...
    private function deleteApplication(): void {
        ...
        if ($this->source !== 'back' && $existing['status'] !== 'pending') {
            http_response_code(422);
            ob_end_clean();
        ...
        ob_end_clean();
        $message = $this->source === 'back' ? 'Application deleted.' : 'Application withdrawn.';
