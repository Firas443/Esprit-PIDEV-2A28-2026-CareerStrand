...
class EventsController {
    ...
    {
        return "SELECT e.eventId, e.managerId, e.sponsorId, e.title, e.description, e.type,
                       e.location, e.date, e.capacity, e.status, e.createdAt,
                       COALESCE(e.tags, '') AS tags, COALESCE(e.organiser, '') AS organiser,
                       COALESCE(e.time, '') AS time, COALESCE(e.eventMode, 'Online') AS eventMode,
                       COALESCE(e.duration, 0) AS duration, COALESCE(e.formLink, '') AS formLink
                FROM event e";
    }

    // ── READ ALL ───────────────────────────────────── 
    public function listerEvents(): array {
        $sql = $this->baseSelect() . " ORDER BY e.date DESC";
    ...
    public function getEventById(int $id): ?Event {
        $sql = $this->baseSelect() . " WHERE e.eventId = :id";
        $db  = config::getConnexion();
        try {
            $req = $db->prepare($sql);
            $req->execute([':id' => $id]);
    ...
    public function getEventsByType(string $type): array {
        $sql = $this->baseSelect() . " WHERE e.type = :type ORDER BY e.date DESC";
        $db  = config::getConnexion();
        try {
            $req = $db->prepare($sql);
    ...
    public function getUpcomingEvents(): array {
        $sql = $this->baseSelect() . " WHERE e.status = 'Upcoming' ORDER BY e.date ASC";
        $db  = config::getConnexion();
        try {
            $stmt   = $db->query($sql);
            $events = [];
    ...
    public function searchEvents(string $keyword): array {
        $sql = $this->baseSelect() . "
                WHERE e.title LIKE :kw OR e.description LIKE :kw
                   OR e.location LIKE :kw OR e.type LIKE :kw
